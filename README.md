...
- 텍스트와 특수문자를 활용한 그래픽과 스킬 이펙트 애니메이션
...
<코딩에 대한 지식이 없는 21살때, 독학을 조금하고 만든 코드라서 가독성이 엉망인점 양해 부탁드립니다.>
...
- 캐릭터를 텍스트 '옷' 으로 표현하였으며, 각종 스킬을 사용할경우 이곳에서 이펙트 애니메이션이 재생됩니다.
...
- 컴퓨터의 스킬 이펙트 재생이 끝나면, 플레이어가 받은 피해가 얼마나 되는지 하단 설명 문구에 출력되며, 상단의 체력 등 능력치 정보가 갱신됩니다.
